...
final class Contract extends DiscordPart implements \JsonSerializable
{
    ...
    // keep fillable behaviour inherited from Discord\Parts\Part

    protected function setStartedAtAttribute($value): void
    {
        if (is_string($value)) {
            try {
    ...
    protected function setResolvedAtAttribute($value): void
    {
        if (is_string($value)) {
            try {
        ...
        $this->attributes['resolved_at'] = $value;
    }

    protected function setOwnerAttribute($value): void
    {
        if (is_array($value)) {
            $this->attributes['owner'] = PartFactory::create('character', $value);
        } else {
